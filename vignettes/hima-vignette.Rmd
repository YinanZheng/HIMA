...
output:
  html_document:
    ...
    code_folding: none
vignette: >
  %\VignetteIndexEntry{High-Dimensional Mediation Analysis with HIMA}
...
- **Survival Data**: For survival analysis, use the format `Surv(Status, Time) ~ Exposure + Covariates`. See data examples `SurvivalData$PhenoData` for more details.
...
- In actual data analysis, `scale` is almost always `TRUE`.
- If the `HIMA` package is used for simulation studies, the `scale` function should be set to `FALSE`, as applying the scale transformation would alter the true values of the model's parameters.
Please note that in actual data analysis, `scale` is almost always `TRUE`. If the `HIMA` package is used for simulation studies, the `scale` function should be set to `FALSE`, as applying the scale transformation would alter the true values of the model's parameters.
...
hima_survival.fit <- hima(
  Surv(Status, Time) ~ Treatment + Sex + Age,
